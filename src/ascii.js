...
function Ascii(source, timeout) {
    ...
    this._source.on('data', function (data) {
        ...
        // var startIndex = that._buffer.indexOf(':');
        var startIndex = indexOfBuffer(that._buffer, new Buffer(':'));
        if (startIndex === -1) {
        ...
        if (startIndex > 0) {
            that._buffer = that._buffer.slice(startIndex);
        }
        that._setupTimer();

        // console.log(that._buffer);
        // do we have the complete message (i.e. are the end delimiters there)
        if (indexOfBuffer(that._buffer, new Buffer('\r\n')) >= 0) {
            that._emit();
        } else {
